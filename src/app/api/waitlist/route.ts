...
/**
 * True if `fid` follows the /columns Farcaster channel.
 *
 * "Following" a channel (subscribing to see it in feed) is distinct from
 * being an explicit channel member/host. Neynar's user/channels endpoint
 * returns channel subscriptions, so this is the correct check.
 * Paginates up to 5 pages (500 channels) in case the user follows many.
 */
