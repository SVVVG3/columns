...
export function ConversationPanel({ viewerFid }: ConversationPanelProps) {
  ...
  const { selectedCastHash, conversationHistory, closeConversation, goBack, openConversation } = useUiStore();
  const isOpen = !!selectedCastHash;
  const canGoBack = conversationHistory.length > 1;
  const scrollRef = useRef<HTMLDivElement>(null);

  // Scroll to top whenever the displayed cast changes
  useEffect(() => {
    if (scrollRef.current) scrollRef.current.scrollTop = 0;
  }, [selectedCastHash]);

  // Escape: go back if there's history, otherwise close
  useEffect(() => {
  ...
  }, [canGoBack, goBack, closeConversation]);

  const { data, isLoading, isError } = useQuery({
    queryKey: ["conversation", selectedCastHash],
    queryFn: async () => {
      const res = await fetch(`/api/cast/${encodeURIComponent(selectedCastHash!)}/conversation`);
      if (!res.ok) throw new Error("Failed to load conversation");
      return res.json();
    },
    enabled: !!selectedCastHash,
    staleTime: 0, // Always refetch when panel opens so viewer_context (liked/recasted state) is current
  });
  ...
  return (
    <>
      {/* Slide-in panel */}
      <div
        className={`fixed top-0 right-0 bottom-0 w-[380px] flex flex-col bg-[var(--background)] border-l border-[var(--border)] z-40 transition-transform duration-200 ease-out ${
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)] shrink-0">
          <div className="flex items-center gap-2">
            {canGoBack ? (
              <button
                onClick={goBack}
                className="text-[var(--muted)] hover:text-[var(--foreground)] transition-colors p-1 -ml-1 rounded-lg"
                title="Back (Esc)"
              >
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto feed-scroll">
          ...
          {rootCast && !isLoading && (
            <>
              {/* Root cast — rendered full-fidelity via CastCard */}
              <div className="border-b-2 border-[var(--border)]">
                <CastCard cast={rootCast} viewerFid={viewerFid} threadRootHash={selectedCastHash ?? undefined} />
              </div>
