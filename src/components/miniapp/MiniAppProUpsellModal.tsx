...
interface MiniAppProUpsellModalProps {
  ...
  communityUrl: string;
  /** Short headline for why this feature needs Pro. */
  featureTitle: string;
...
export function MiniAppProUpsellModal({
  ...
  useEffect(() => {
    if (open) checkWaitlistStatus();
  }, [open, checkWaitlistStatus]);
      ...
      >
        ...
        <div className="px-4 pb-4 space-y-2 border-t border-[var(--border)] pt-3">
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-full py-2 text-xs text-[var(--muted)] hover:text-[var(--foreground)]"
          >
            Not now
          </button>
        </div>
